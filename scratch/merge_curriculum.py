...
# Load generated math lessons from script
from generate_all_math_lessons import toan_subject, tiengviet_subject, tienganh_subject

# Read existing curriculumData.js to extract Science (khoahoc) object
with open(r"d:\OneDrive\2025 - 2026\Hoc tap Lop 5\src\data\curriculumData.js", "r", encoding="utf-8") as f:
    content = f.read()

# Find khoahoc block
khoahoc_match = re.search(r"\{\s*id:\s*'khoahoc'[\s\S]*?\n  \}", content)

if khoahoc_match:
    khoahoc_str = khoahoc_match.group(0)
else:
    print("Warning: khoahoc block not found via regex, using fallback.")
    khoahoc_str = ""
...
# Extract existing Khoa hoc block from curriculumData.js if possible, or construct standard object
# We know science was in curriculumData.js starting from line 197
with open(r"d:\OneDrive\2025 - 2026\Hoc tap Lop 5\src\data\curriculumData.js", "r", encoding="utf-8") as f:
    orig_text = f.read()

kh_start = orig_text.find("id: 'khoahoc'")
if kh_start != -1:
    # Find start of object brace before 'id: khoahoc'
    brace_start = orig_text.rfind("{", 0, kh_start)
    # Find closing array end bracket
...
print("curriculumData.js written successfully! Size:", len(final_js))
