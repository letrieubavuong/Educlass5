...
if kh_start != -1:
    ...
    if th_start != -1:
        ...
        # Fallback to end of khoahoc
        next_obj = orig_text.find('{', kh_start)
        kh_block = orig_text[brace_start:next_obj].strip()
...
# Verify Science block cleanly
if not kh_block or "tinhoc" in kh_block:
    # Build clean science object string from previous data or regex
    kh_match = re.search(r"(\{\s*id:\s*'khoahoc'[\s\S]*?\n  \})", orig_text)
    if kh_match:
        kh_block = kh_match.group(1)
...
print("Deduplicated curriculumData.js written cleanly!")
